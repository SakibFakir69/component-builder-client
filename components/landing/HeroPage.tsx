...
// --- DATA & MAPPING ---
type FrameworkId = 'html' | 'react' | 'vue' | 'svelte' | 'jquery';

const iconMap: Record<FrameworkId, ({ className }: { className?: string }) => JSX.Element> = {
    html: HtmlIcon,
    react: ReactIcon,
    vue: VueIcon,
    svelte: SvelteIcon,
...
const codeImageMap: Record<FrameworkId, string> = {
    react: 'https://placehold.co/1200x600/61DAFB/1E293B?text=React+Code+Snippet',
    vue: 'https://placehold.co/1200x600/41B883/1E293B?text=Vue+Code+Snippet',
    svelte: 'https://placehold.co/1200x600/FF3E00/1E293B?text=Svelte+Code+Snippet',
    jquery: 'https://placehold.co/1200x600/0769AD/1E293B?text=jQuery+Code+Snippet',
    html: 'https://placehold.co/1200x600/E34F26/1E293B?text=HTML+Code+Snippet',
};
...
interface CodeDisplayProps {
    isVisible: boolean;
    activeTab: FrameworkId; // Added activeTab prop
}

const CodeDisplay = ({ isVisible, activeTab }: CodeDisplayProps) => {
    // Get the image URL for the active tab
    const imageUrl = codeImageMap[activeTab];

    return (
        // Resetting the transition opacity and translateY on tab change
        <div key={activeTab} className={`relative max-w-5xl mx-auto px-4 transition-all duration-500 ease-in-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="relative bg-white dark:bg-gray-950 rounded-lg sm:rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-colors duration-300">
                ...
                <div className="bg-gray-900">
                    <img
                        ...
                        alt={`Code snippet for ${activeTab} setup`}
                        className="w-full h-auto object-cover"
                        onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            target.onerror = null;
                            target.src = 'https://placehold.co/1200x600/111827/FFFFFF?text=Code+Snippet+Image+Unavailable';
                        }}
                    />
                </div>
...
export default function Hero5() {
    const [activeTab, setActiveTab] = useState<FrameworkId>('react');
    ...
    const frameworks: Array<{ id: FrameworkId; name: string }> = [
        { id: 'html', name: 'HTML' },
        { id: 'react', name: 'React' },
        { id: 'vue', name: 'Vue' },
    ...
    return (
        <div className="font-sans">
            <style jsx global>{` /* Using jsx global for the keyframes for better Next.js compatibility */
        @keyframes slide-up {
  ...
  <p className="mt-8 text-base sm:text-lg text-gray-500 dark:text-gray-400">
    Powered by GEMNI 2.5 Flash • Over 120,000 components generated this month
                            ...
                            {frameworks.map(fw => (
                                <div key={fw.id}> {/* 🔑 Added missing key prop here */}
                                    <FrameworkPill
                                        ...
                                        onClick={() => {
                                            // 1. Set to invisible (trigger fade out/slide down)
                                            setIsVisible(false);
                                            // 2. Change the tab after a short delay (shorter than the transition)
                                            setTimeout(() => setActiveTab(fw.id), 100);
                                            // 3. Set to visible (trigger fade in/slide up for new content)
                    ...
                    <div className={`mb-8 sm:mb-12 ${isVisible ? 'animate-slide-up animation-delay-200' : 'opacity-0'}`}>
                        {/* 🚀 Passed activeTab to CodeDisplay to make it dynamic */}
                        <CodeDisplay isVisible={isVisible} activeTab={activeTab} />
                    </div>
