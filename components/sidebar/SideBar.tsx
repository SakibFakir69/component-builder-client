...
import { useGetMeQuery, useLoginUserMutation, useLogoutUserMutation } from "@/lib/api/baseApi";
...
console.log({email,role,name}, ' get me')
          ...
          >
            ...
            {isOpen && (
              <div className="overflow-hidden flex-1 animate-in fade-in duration-300">
                ...
                <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold text-left">
                  {role}
                </p>
              </div>
            )}
            {isOpen && (
              <button className="text-slate-500 hover:text-rose-500 transition-colors">
                <LogOut onClick={handelLogout} size={16} />
              </button>
            )}
          </div>
        </div>
